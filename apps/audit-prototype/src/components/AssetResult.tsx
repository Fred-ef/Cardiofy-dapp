...
import { verifyAsset, type VerifyResult } from '../lib/verifier.js';
import type { ApiError } from '../lib/api-client.js';
import { type AssetDto } from '../lib/api-client.js';
...
export function AssetResult({ chainInfo }: { chainInfo: ChainInfoDto }) {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<VerifyResult<AssetDto> | null>(null);
  ...
  async function onVerify(id: string, token: string | undefined) {
    ...
    setResult(null);
    try {
      setResult(await verifyAsset(chainInfo, id, token));
    } catch (e) {
  ...
  return (
    <section className="result-panel">
      <LookupForm idLabel="Asset ID" idPlaceholder="es. asset-123" onSubmit={onVerify} loading={loading} />
      {error && <ErrorPanel error={error} />}
      {result && (
        <div className="result">
          <h3>
            Asset <span className="mono">{result.api.assetId}</span> <VerdictBadge verdict={result.overall} />
          </h3>
